...
u8 fuzz_one(afl_state_t *afl) {
  ...
  if (afl->limit_time_sig <= 0) { 
      ...
      if (afl->is_api_mode) { 
          ...
          afl->api = (struct api_entry*)ck_alloc(sizeof(struct api_entry));

          key_val_lv_1 = fuzz_one_api(afl);

          clean_entries(afl->api);
...
u8 fuzz_one_api(afl_state_t *afl) {
  u32 len, temp_len;
  u32 j;
  u32 i;
  u32 k;
   ...
   * TRIMMING *
   ************/
  /* Here we need to rewrite this function, just disable it */
  /*
  if (unlikely(!afl->non_instrumented_mode && !afl->queue_cur->trim_done &&
               !afl->disable_trim)) {
    u32 old_len = afl->queue_cur->len;
...
#define FLIP_BIT(_ar, _b)                   \
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    if (afl->api->args_info[i].skip_fuzz) continue;
    sprintf(stage_name_t, "bitflip 1/1(%d)", i);
    sprintf(stage_short_t, "flip1 %d-%d", i, afl->api->argument_num);

    afl->stage_short = stage_short_t;
    afl->stage_name = stage_name_t;
    afl->stage_max = afl->api->args_info[i].len << 3;

    u8 *value_buf = afl->api->args_info[i].value->valuestring;
    u8 *value_buf_bak = malloc(strlen(value_buf) + 1);
    memcpy(value_buf_bak, value_buf, strlen(value_buf) + 1);

    orig_hit_cnt = afl->queued_items + afl->saved_crashes;

    /* Now flip bits. */

    if (afl->api->args_info[i].type == FIXED_DATA ||
        afl->api->args_info[i].type == (FIXED_DATA | STRUCT_DATA)) {
      u64 num = 0;
      u8 *t = value_buf;
      num = fetch_right_val(afl->api, t, i);

      u8 temp[21];
      num = num & ((u64)-1 >> (64 - afl->api->args_info[i].len * 8));

      for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max;
           afl->stage_cur++) {
        afl->stage_cur_byte = afl->stage_cur >> 3;

        FLIP_BIT(&num, afl->stage_cur);
        ...
        cJSON_SetValuestring(afl->api->args_info[i].value, temp);

        u8 *new_cjson = cJSON_Print(afl->api->monitor_json);

        if (common_fuzz_stuff(afl, new_cjson, strlen(new_cjson)))
          goto abandon_entry;
      ...
      cJSON_SetValuestring(afl->api->args_info[i].value, value_buf_bak);
      ...
      for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max;
        ...
        FLIP_BIT(value_buf, afl->stage_cur);
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    if (afl->api->args_info[i].skip_fuzz) continue;
    sprintf(stage_name_t, "bitflip 2/1(%d)", i);
    sprintf(stage_short_t, "flip2 %d-%d", i, afl->api->argument_num);

    afl->stage_short = stage_short_t;
    afl->stage_name = stage_name_t;
    afl->stage_max = (afl->api->args_info[i].len << 3) - 1;
    ...
    if (afl->api->args_info[i].type == FIXED_DATA ||
      ...
      cJSON_SetValuestring(afl->api->args_info[i].value, value_buf_bak);
        ...
        FLIP_BIT(value_buf, afl->stage_cur);
        FLIP_BIT(value_buf, afl->stage_cur + 1);
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    if (afl->api->args_info[i].skip_fuzz) continue;
    sprintf(stage_name_t, "bitflip 4/1(%d)", i);
    sprintf(stage_short_t, "flip4 %d-%d", i, afl->api->argument_num);

    afl->stage_short = stage_short_t;
    afl->stage_name = stage_name_t;
    afl->stage_max = (afl->api->args_info[i].len << 3) - 3;
    ...
    if (afl->api->args_info[i].type == FIXED_DATA ||
      ...
      for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max;
        ...
        FLIP_BIT(&num, afl->stage_cur);
        FLIP_BIT(&num, afl->stage_cur + 1);
        FLIP_BIT(&num, afl->stage_cur + 2);
        FLIP_BIT(&num, afl->stage_cur + 3);
      ...
      for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max;
        ...
        FLIP_BIT(value_buf, afl->stage_cur);
        FLIP_BIT(value_buf, afl->stage_cur + 1);
        FLIP_BIT(value_buf, afl->stage_cur + 2);
        FLIP_BIT(value_buf, afl->stage_cur + 3);
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    if (afl->api->args_info[i].skip_fuzz) continue;
    sprintf(stage_name_t, "bitflip 8/8(%d)", i);
    sprintf(stage_short_t, "flip8 %d-%d", i, afl->api->argument_num);

    afl->stage_short = stage_short_t;
    afl->stage_name = stage_name_t;
    afl->stage_max = afl->api->args_info[i].len;
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    if (afl->api->args_info[i].len < 2) continue;
    if (afl->api->args_info[i].skip_fuzz) continue;

    sprintf(stage_name_t, "bitflip 16/8(%d)", i);
    sprintf(stage_short_t, "flip16 %d-%d", i, afl->api->argument_num);

    afl->stage_short = stage_short_t;
    afl->stage_name = stage_name_t;
    afl->stage_max = afl->api->args_info[i].len - 1;
    ...
    if (afl->api->args_info[i].type == FIXED_DATA ||
      ...
      for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max;
        ...
        *(u16 *)(value_buf + afl->stage_cur) ^= 0xFFFF;
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    if (afl->api->args_info[i].len < 4) continue;
    if (afl->api->args_info[i].skip_fuzz) continue;
    sprintf(stage_name_t, "bitflip 32/8(%d)", i);
    sprintf(stage_short_t, "flip32 %d-%d", i, afl->api->argument_num);
    ...
    if (afl->api->args_info[i].type == FIXED_DATA ||
      ...
      for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max;
        ...
        *(u32 *)(value_buf + afl->stage_cur) ^= 0xFFFFFFFF;
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    if (afl->api->args_info[i].skip_fuzz) continue;
    sprintf(stage_name_t, "arith 8/8(%d)", i);
    sprintf(stage_short_t, "arith8 %d-%d", i, afl->api->argument_num);
    ...
    if (afl->api->args_info[i].type == FIXED_DATA ||
      ...
      for (afl->stage_cur = 1; afl->stage_cur <= afl->stage_max;
           afl->stage_cur++) {
        afl->stage_cur_byte = afl->stage_cur;

        u64 num_s = num + afl->stage_cur;
        num_s = num_s & ((u64)-1 >> (64 - afl->api->args_info[i].len * 8));
        ...
        new_cjson = cJSON_Print(afl->api->monitor_json);
      ...
      afl->stage_max = 2 * afl->api->args_info[i].len * ARITH_MAX;

      for (j = 0; j < afl->api->args_info[i].len; j++) {
        u8 orig = value_buf[j];

        afl->stage_cur_byte = j;

        for (k = 1; k <= ARITH_MAX; k++) {
          u8 r = orig ^ (orig + k);
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    ...
    sprintf(stage_name_t, "arith 16/8(%d)", i);
    sprintf(stage_short_t, "arith16 %d-%d", i, afl->api->argument_num);

    afl->stage_short = stage_short_t;
    afl->stage_name = stage_name_t;
    afl->stage_cur = 0;
    afl->stage_max = 4 * (afl->api->args_info[i].len - 1) * ARITH_MAX;
    ...
    for (j = 0; j < afl->api->args_info[i].len - 1; j++) {
      u16 orig = *(u16 *)(value_buf + j);

      afl->stage_cur_byte = j;

      for (k = 1; k <= ARITH_MAX; k++) {
        u16 r1 = orig ^ (orig + k), r2 = orig ^ (orig - k),
            r3 = orig ^ SWAP16(SWAP16(orig) + k),
            r4 = orig ^ SWAP16(SWAP16(orig) - k);

        afl->stage_val_type = STAGE_VAL_LE;

        if ((orig & 0xff) + k > 0xff && !could_be_bitflip(r1)) {
          afl->stage_cur_val = k;
          *(u16 *)(value_buf + j) = orig + k;
        ...
        if ((orig & 0xff) < k && !could_be_bitflip(r2)) {
          afl->stage_cur_val = -k;
          *(u16 *)(value_buf + j) = orig - k;
        ...
        if ((orig >> 8) + k > 0xff && !could_be_bitflip(r3)) {
          afl->stage_cur_val = k;
          *(u16 *)(value_buf + j) = SWAP16(SWAP16(orig) + k);
        ...
        if ((orig >> 8) < k && !could_be_bitflip(r4)) {
          afl->stage_cur_val = -k;
          *(u16 *)(value_buf + j) = SWAP16(SWAP16(orig) - k);
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    ...
    sprintf(stage_name_t, "arith 32/8(%d)", i);
    sprintf(stage_short_t, "arith32 %d-%d", i, afl->api->argument_num);

    afl->stage_short = stage_short_t;
    afl->stage_name = stage_name_t;
    afl->stage_cur = 0;
    afl->stage_max = 4 * (afl->api->args_info[i].len - 3) * ARITH_MAX;
    ...
    for (j = 0; j < afl->api->args_info[i].len - 3; j++) {
      u32 orig = *(u32 *)(value_buf + j);

      afl->stage_cur_byte = j;

      for (k = 1; k <= ARITH_MAX; k++) {
        u32 r1 = orig ^ (orig + k), r2 = orig ^ (orig - k),
            r3 = orig ^ SWAP32(SWAP32(orig) + k),
            r4 = orig ^ SWAP32(SWAP32(orig) - k);

        afl->stage_val_type = STAGE_VAL_LE;

        if ((orig & 0xffff) + k > 0xffff && !could_be_bitflip(r1)) {
          afl->stage_cur_val = k;
          *(u32 *)(value_buf + j) = orig + k;
        ...
        if ((orig & 0xffff) < k && !could_be_bitflip(r2)) {
          afl->stage_cur_val = -k;
          *(u32 *)(value_buf + j) = orig - k;
        ...
        if ((SWAP32(orig) & 0xffff) + k > 0xffff && !could_be_bitflip(r3)) {
          afl->stage_cur_val = k;
          *(u32 *)(value_buf + j) = SWAP32(SWAP32(orig) + k);
        ...
        if ((SWAP32(orig) & 0xffff) < k && !could_be_bitflip(r4)) {
          afl->stage_cur_val = -k;
          *(u32 *)(value_buf + j) = SWAP32(SWAP32(orig) - k);
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    sprintf(stage_name_t, "interest 8/8(%d)", i);
    sprintf(stage_short_t, "int8 %d-%d", i, afl->api->argument_num);
    if (afl->api->args_info[i].skip_fuzz) continue;
    ...
    if (afl->api->args_info[i].type == FIXED_DATA ||
        afl->api->args_info[i].type == (FIXED_DATA | STRUCT_DATA)) {
      if (afl->api->args_info[i].len != 1) continue;
      afl->stage_max = sizeof(interesting_8);
      ...
      for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max;
           afl->stage_cur++) {
        u64 num_t = interesting_8[afl->stage_cur];
      ...
      afl->stage_max = afl->api->args_info[i].len * sizeof(interesting_8);
      afl->stage_val_type = STAGE_VAL_LE;
      ...
      for (j = 0; j < afl->api->args_info[i].len; j++) {
        ...
        for (k = 0; k < sizeof(INTERESTING_8); k++) {
          if (could_be_bitflip(orig ^ (u8)interesting_8[k]) ||
              could_be_arith(orig, (u8)interesting_8[k], 1)) {
            afl->stage_max--;
            continue;
          }

          afl->stage_cur_val = interesting_8[k];
          value_buf[j] = interesting_8[k];
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    ...
    sprintf(stage_name_t, "interest 16/8(%d)", i);
    sprintf(stage_short_t, "int16 %d-%d", i, afl->api->argument_num);
    ...
    if (afl->api->args_info[i].type == FIXED_DATA ||
      ...
      for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max;
           afl->stage_cur++) {
        u64 num_t = interesting_16[afl->stage_cur];
      ...
      afl->stage_max =
          2 * (afl->api->args_info[i].len - 1) * (sizeof(interesting_16) >> 1);
      ...
      for (j = 0; j < afl->api->args_info[i].len - 1; j++) {
        ...
        for (k = 0; k < sizeof(INTERESTING_16) / 2; k++) {
          afl->stage_cur_val = interesting_16[k];

          if (!could_be_bitflip(orig ^ (u16)interesting_16[k]) &&
              !could_be_arith(orig, (u16)interesting_16[k], 2) &&
              !could_be_interest(orig, (u16)interesting_16[k], 2, 0)) {
            afl->stage_val_type = STAGE_VAL_LE;

            *(u16 *)(value_buf + j) = interesting_16[k];
          ...
          if ((u16)interesting_16[k] != SWAP16(interesting_16[k]) &&
              !could_be_bitflip(orig ^ SWAP16(interesting_16[k])) &&
              !could_be_arith(orig, SWAP16(interesting_16[k]), 2) &&
              !could_be_interest(orig, SWAP16(interesting_16[k]), 2, 1)) {
            afl->stage_val_type = STAGE_VAL_BE;

            *(u16 *)(value_buf + j) = SWAP16(interesting_16[k]);
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    ...
    sprintf(stage_name_t, "interest 32/8(%d)", i);
    sprintf(stage_short_t, "int32 %d-%d", i, afl->api->argument_num);
    ...
    if (afl->api->args_info[i].type == FIXED_DATA ||
        afl->api->args_info[i].type == (FIXED_DATA | STRUCT_DATA)) {
      if (afl->api->args_info[i].len < 4) continue;
      afl->stage_max = sizeof(interesting_32);

      u8 temp[21];

      for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max;
           afl->stage_cur++) {
        u64 num_t = interesting_32[afl->stage_cur];
      ...
      afl->stage_max =
          2 * (afl->api->args_info[i].len - 3) * (sizeof(interesting_32) >> 2);
      ...
      for (j = 0; j < afl->api->args_info[i].len - 3; j++) {
        ...
        for (k = 0; k < sizeof(interesting_32) / 4; k++) {
          afl->stage_cur_val = interesting_32[k];

          if (!could_be_bitflip(orig ^ (u32)interesting_32[k]) &&
              !could_be_arith(orig, interesting_32[k], 4) &&
              !could_be_interest(orig, interesting_32[k], 4, 0)) {
            afl->stage_val_type = STAGE_VAL_LE;

            *(u32 *)(value_buf + j) = interesting_32[k];
          ...
          if ((u32)interesting_32[k] != SWAP32(interesting_32[k]) &&
              !could_be_bitflip(orig ^ SWAP32(interesting_32[k])) &&
              !could_be_arith(orig, SWAP32(interesting_32[k]), 4) &&
              !could_be_interest(orig, SWAP32(interesting_32[k]), 4, 1)) {
            afl->stage_val_type = STAGE_VAL_BE;

            *(u32 *)(value_buf + j) = SWAP32(interesting_32[k]);
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    ...
    sprintf(stage_name_t, "user extras (over)(%d)", i);
    sprintf(stage_short_t, "ext_UO %d-%d", i, afl->api->argument_num);

    afl->stage_short = stage_short_t;
    afl->stage_name = stage_name_t;
    afl->stage_cur = 0;
    afl->stage_max = afl->extras_cnt * afl->api->args_info[i].len;
    ...
    for (j = 0; j < afl->api->args_info[i].len; j++) {
      ...
      for (k = 0; k < afl->extras_cnt; k++) {
        if ((afl->extras_cnt > MAX_DET_EXTRAS &&
             rand_below(afl, afl->extras_cnt) >= MAX_DET_EXTRAS) ||
            afl->extras[k].len > afl->api->args_info[i].len - j ||
            !memcmp(afl->extras[k].data, value_buf + j, afl->extras[k].len)) {
          afl->stage_max--;
          continue;
        }

        last_len = afl->extras[k].len;
        memcpy(value_buf + j, afl->extras[k].data, last_len);
      ...
      memcpy(value_buf + j, value_buf_bak + j, last_len);
    }
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    ...
    sprintf(stage_name_t, "user extras (insert)(%d)", i);
    sprintf(stage_short_t, "ext_UI %d-%d", i, afl->api->argument_num);

    afl->stage_short = stage_short_t;
    afl->stage_name = stage_name_t;
    afl->stage_cur = 0;
    afl->stage_max = afl->extras_cnt * (afl->api->args_info[i].len + 1);
    ...
    ex_tmp = ck_alloc(afl->api->args_info[i].len + MAX_DICT_FILE);

    for (j = 0; j < afl->api->args_info[i].len; j++) {
      afl->stage_cur_byte = j;

      for (k = 0; k < afl->extras_cnt; k++) {
        if (afl->api->args_info[i].len + afl->extras[k].len > MAX_FILE) {
          afl->stage_max--;
          continue;
        }

        /* Insert token */
        memcpy(ex_tmp + j, afl->extras[k].data, afl->extras[k].len);

        /* Copy tail */
        memcpy(ex_tmp + j + afl->extras[k].len, value_buf_bak + j,
               afl->api->args_info[i].len - j);

        cJSON_SetValuestring(afl->api->args_info[i].value, ex_tmp);
...
skip_user_extras:
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    ...
    sprintf(stage_name_t, "auto extras (over)(%d)", i);
    sprintf(stage_short_t, "ext_AO %d-%d", i, afl->api->argument_num);

    afl->stage_short = stage_short_t;
    afl->stage_name = stage_name_t;
    afl->stage_cur = 0;
    afl->stage_max =
        MIN(afl->a_extras_cnt, USE_AUTO_EXTRAS) * afl->api->args_info[i].len;
    ...
    for (j = 0; j < afl->api->args_info[i].len; j++) {
      ...
      for (k = 0; k < MIN(afl->a_extras_cnt, USE_AUTO_EXTRAS); k++) {
        if (afl->a_extras[k].len > afl->api->args_info[i].len - j ||
            !memcmp(afl->a_extras[k].data, value_buf + j,
                    afl->a_extras[k].len)) {
          afl->stage_max--;
          continue;
        }
        ...
        last_len = afl->a_extras[j].len;
        memcpy(value_buf + j, afl->a_extras[k].data, last_len);
...
havoc_stage:
  ...
  for (i = 0; i < afl->api->argument_num; i++) {
    ...
    len = afl->api->args_info[i].len;

    if (afl->api->args_info[i].type == UNFIXED_DATA ||
        afl->api->args_info[i].type == (UNFIXED_DATA | STRUCT_DATA)) {
havoc_stage_arg:

      afl->stage_cur_byte = -1;

      sprintf(stage_name_t, "havoc(%d)", i);
      sprintf(stage_short_t, "havoc %d-%d", i, afl->api->argument_num);
      ...
      for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max;
        ...
        for (j = 0; j < use_stacking; j++) {
          switch (rand_below(afl, 15 +
                             ((afl->extras_cnt + afl->a_extras_cnt) ? 2 : 0))) {
            case 0:

              FLIP_BIT(value_buf, rand_below(afl, temp_len << 3));
              break;

            case 1:

              value_buf[rand_below(afl, temp_len)] =
                  interesting_8[rand_below(afl, sizeof(interesting_8))];
              break;
            ...
            case 2:
              ...
              if (rand_below(afl, 2)) {
                *(u16 *)(value_buf + rand_below(afl, temp_len - 1)) =
                    interesting_16[rand_below(afl, sizeof(interesting_16) >> 1)];
              } else {
                *(u16 *)(value_buf + rand_below(afl, temp_len - 1)) =
            ...
            case 3:
              ...
              if (rand_below(afl, 2)) {
                *(u32 *)(value_buf + rand_below(afl, temp_len - 3)) =
                    interesting_32[rand_below(afl, sizeof(interesting_32) >> 2)];
              } else {
                *(u32 *)(value_buf + rand_below(afl, temp_len - 3)) =
            ...
            case 4:
              ...
              value_buf[rand_below(afl, temp_len)] -= 1 + rand_below(afl, ARITH_MAX);
              break;
            ...
            case 6:
              ...
              if (rand_below(afl, 2)) {
                ...
                *(u16 *)(value_buf + pos) -= 1 + rand_below(afl, ARITH_MAX);
                ...
                *(u16 *)(value_buf + pos) =
                    SWAP16(SWAP16(*(u16 *)(value_buf + pos)) - num);
              }
            ...
            case 8:
              ...
              if (rand_below(afl, 2)) {
                ...
                *(u32 *)(value_buf + pos) -= 1 + rand_below(afl, ARITH_MAX);
                ...
                *(u32 *)(value_buf + pos) =
                    SWAP32(SWAP32(*(u32 *)(value_buf + pos)) - num);
              }
            ...
            case 10:
              ...
              value_buf[rand_below(afl, temp_len)] ^= 1 + rand_below(afl, 255);
              break;
            ...
            case 11 ... 12: {
              ...
              memmove(value_buf + del_from, value_buf + del_from + del_len,
                      temp_len - del_from - del_len);

              temp_len -= del_len;

              value_buf[temp_len] = 0;
            ...
            case 13:
              ...
              if (temp_len + HAVOC_BLK_XL < MAX_FILE) {
                ...
                new_buf = malloc(temp_len + clone_len + 1);
                new_buf[temp_len + clone_len] = 0;

                /* Head */

                memcpy(new_buf, value_buf, clone_to);

                /* Inserted part */

                if (actually_clone)
                  memcpy(new_buf + clone_to, value_buf + clone_from, clone_len);
                else
                  memset(new_buf + clone_to,
                         rand_below(afl, 2) ? rand_below(afl, 256) : value_buf[rand_below(afl, temp_len)], clone_len);

                /* Tail */
                memcpy(new_buf + clone_to + clone_len, value_buf + clone_to,
                       temp_len - clone_to);

                free(value_buf);
                value_buf = afl->api->args_info[i].value->valuestring = new_buf;
                temp_len += clone_len;
              }
            ...
            case 14: {
              ...
              if (rand_below(afl, 4)) {
                if (copy_from != copy_to)
                  memmove(value_buf + copy_to, value_buf + copy_from, copy_len);

              } else
                memset(value_buf + copy_to,
                       rand_below(afl, 2) ? rand_below(afl, 256) : value_buf[rand_below(afl, temp_len)], copy_len);
            ...
            case 15: {
              if (!afl->extras_cnt || (afl->a_extras_cnt && rand_below(afl, 2))) {
                ...
                memcpy(value_buf + insert_at, afl->a_extras[use_extra].data,
                       extra_len);
                ...
                insert_at = rand_below(afl, temp_len - extra_len + 1);
                memcpy(value_buf + insert_at, afl->extras[use_extra].data,
                       extra_len);
              }
            ...
            case 16: {
              ...
              if (!afl->extras_cnt || (afl->a_extras_cnt && rand_below(afl, 2))) {
                ...
                new_buf = malloc(temp_len + extra_len + 1);
                new_buf[temp_len + extra_len] = 0;

                memcpy(new_buf, value_buf, insert_at);
                ...
                memcpy(new_buf + insert_at, afl->extras[use_extra].data,
                       extra_len);
              }

              memcpy(new_buf + insert_at + extra_len, value_buf + insert_at,
                     temp_len - insert_at);
        ...
        if (temp_len < afl->api->args_info[i].len) {
          afl->api->args_info[i].value->valuestring =
              realloc(value_buf, afl->api->args_info[i].len + 1);
          value_buf = afl->api->args_info[i].value->valuestring;
        }

        value_buf[afl->api->args_info[i].len] = 0;
        temp_len = afl->api->args_info[i].len;
        memcpy(value_buf, value_buf_bak, temp_len + 1);
      ...
      */
      for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max;
        ...
        if (!afl->api->args_info[i].domain.total_n) {
          num_t = (u64)(rand_below(afl, -1) + 1)
                      << 32 + (u64)(rand_below(afl, -1) + 1);
          num_t = num_t & ((u64)-1 >> (64 - afl->api->args_info[i].len * 8));
        } else {
          /*
           *  Apply argument constraints:
           *  90% of valid input; 10% of invalid input
           */
          num_t = gen_rand(&afl->api->args_info[i].domain,
                           afl->api->args_info[i].len);
        }
